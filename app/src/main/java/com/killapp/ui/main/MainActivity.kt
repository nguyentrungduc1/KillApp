...
class MainActivity : AppCompatActivity() {
        ...
        override fun onReceive(context: Context, intent: Intent) {
            ...
            when (state) {
                ...
                KillAccessibilityService.STATE_STEP -> {
                    ...
                    val index   = intent.getIntExtra(KillAccessibilityService.EXTRA_STEP_INDEX, progressStep)
                    val total   = intent.getIntExtra(KillAccessibilityService.EXTRA_STEP_TOTAL, totalSteps)

                    val label = when (type) {
                        KillAccessibilityService.STEP_TYPE_FORCE_STOP  -> "⛔ Buộc dừng: $appName"
                        KillAccessibilityService.STEP_TYPE_CLEAR_CACHE -> "🗑 Xoá cache: $appName"
                        KillAccessibilityService.STEP_TYPE_RECENTS     -> "📋 Đóng tất cả tác vụ…"
                        else -> "Đang xử lý: $appName"
                    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent.getBooleanExtra("trigger_kill", false)) {
            // delay đủ để danh sách app load xong
